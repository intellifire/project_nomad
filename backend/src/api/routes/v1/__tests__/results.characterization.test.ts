...
const { getResultById, getModelResultsService } = vi.hoisted(() => {
  const getResultById = vi.fn();
  const getModelResultsService = vi.fn((_engine?: unknown) => ({ getResultById }));
  return { getResultById, getModelResultsService };
});
