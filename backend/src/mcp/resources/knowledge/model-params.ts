...
const MODEL_PARAMETERS = {
  ...
  simulationOptions: {
    ...
    fields: {
      ...
      confidenceInterval: {
        description: 'Probability threshold for perimeter generation (1-90, as integer percentage).',
        type: 'number',
        range: { min: 1, max: 90 },
        hardcodedValue: 1,
        userConfigurable: false,
        notes: [
          'This parameter is hardcoded to 1 by the engine during model execution.',
          'A value of 1 captures all pixels with any burn probability > 0%, producing the full fire spread envelope.',
          'Custom values (1-90) are still accepted by the POST /api/v1/models/{modelId}/perimeters endpoint for advanced use.',
          'UI controls for this parameter were removed in issue #146.',
        ],
      },
      smoothPerimeter: {
        ...
        userConfigurable: false,
        notes: [
          'This parameter is hardcoded to false by the engine during model execution.',
          'Custom values are still accepted by the POST /api/v1/models/{modelId}/perimeters endpoint for advanced use.',
          'UI controls for this parameter were removed in issue #146.',
        ],
      },
    },
